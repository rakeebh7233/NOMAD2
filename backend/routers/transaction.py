...
def show(email, db: Session = Depends(get_db), current_user: schema.UserModel = Depends(oauth2.get_current_user)):
    response = FinanceModel.Transaction.get_transaction_user(db, email)
    if response == None:
...
def show_daily(email, db: Session = Depends(get_db), current_user: schema.UserModel = Depends(oauth2.get_current_user)):
...
def show_weekly(email, start_date, db: Session = Depends(get_db), current_user: schema.UserModel = Depends(oauth2.get_current_user)):
...
def show_monthly(email, start_date, db: Session = Depends(get_db), current_user: schema.UserModel = Depends(oauth2.get_current_user)):
...
def show_yearly(email, start_date, db: Session = Depends(get_db), current_user: schema.UserModel = Depends(oauth2.get_current_user)):
